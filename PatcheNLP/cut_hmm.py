...
class HMM(object):
    def __init__(self,
                 model_file_path=os.path.join(hmm_model_dir, 'hmm_model.pkl'),
                 data_file_path=os.path.join(cut_data_dir,
                                             "CUT@People's Daily199801")):
    ...
    def viterbi(self, text, states, start_p, trans_p, emit_p):
        ...
        for t in range(1, len(text)):
            ...
            nerver_seen: bool = (  # 确认该字不在语料库中
                cur_char not in emit_p['S'] and cur_char not in emit_p['M']
                and cur_char not in emit_p['E']
                and cur_char not in emit_p['B'])
            for state_item in states:
